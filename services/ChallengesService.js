...
class ChallengesService {
   ...
   * @returns {Promise<Object>}
   */
  static async updateChallengeProgress(challengeId, userId, progressData) {
    try {
      const isOnline = await offlineService.checkConnection();
   ...
   */
  static async getActiveChallenges(groupId) {
    try {
      const isOnline = await offlineService.checkConnection();
      ...
      if (isOnline) {
        ...
        const challengesWithParticipants = await Promise.all(
          challenges.map(async challenge => {
            ...
            try {
              ...
              if (rankings && rankings.length > 0) {
                ...
                if (challenge.type === 'distance') {
                  leaderScore = (firstPlace.totalDistance / 1000).toFixed(1); // Convertir mètres en km
                } else if (challenge.type === 'speed') {
                  leaderScore = firstPlace.maxSpeed.toFixed(0);
            ...
            if (challenge.type === 'distance') {
              challengeProgress = leaderScore ? parseFloat(leaderScore) * 1000 : totalProgress; // Utiliser le score du leader si disponible
            ...
            return {
              ...
              bestSpeed: challenge.type === 'speed' && leaderScore ? parseFloat(leaderScore) : (challenge.type === 'speed' ? bestSpeed : null),
  ...
  static async _getLocalChallenges(groupId) {
    try {
  ...
  static async _cacheChallenges(groupId, challenges) {
    try {
  ...
  static async _saveLocalChallenge(challengeData) {
  ...
  static async _updateLocalProgress(challengeId, userId, progressData) {
  ...
  static async _addToLocalCache(challenge) {
    try {
