...
const getVehiclesStorageKey = (userId) => `${VEHICLES_BASE_KEY}:${userId || 'guest'}`;
const getDefaultVehicleStorageKey = (userId) => `${DEFAULT_VEHICLE_ID_BASE_KEY}:${userId || 'guest'}`;
const getVehiclePhotoKey = (vehicleId, userId) => `@vehiclePhoto:${userId || 'guest'}:${vehicleId}`;
const getVehiclePhotoPath = (vehicleId, userId) => `${FileSystem.documentDirectory}vehicles/${userId || 'guest'}/${vehicleId}.jpg`;

export async function cacheVehiclePhoto(remoteUri, vehicleId, userId) {
  try {
...
export async function getCachedVehiclePhoto(vehicleId, userId) {
  try {
...
export async function loadVehicles(userId) {
  try {
    ...
    const cachedVehicles = await Promise.all(
      cachedVehiclesRaw.map(async (vehicle) => {
        if (vehicle?.photoUri) {
          return vehicle;
        }
...
export async function loadDefaultVehicleId(userId) {
  try {
...
export async function setDefaultVehicleId(id, userId) {
  try {
    const storageKey = getDefaultVehicleStorageKey(userId);
    const isOnline = await offlineService.checkConnection();
...
export async function saveVehicle(vehicleData, userId) {
  try {
    const storageKey = getVehiclesStorageKey(userId);
    const isOnline = await offlineService.checkConnection();
...
export async function updateVehicle(vehicleId, updates, userId) {
  try {
    const storageKey = getVehiclesStorageKey(userId);
    const isOnline = await offlineService.checkConnection();
...
export function getVehicleIconByType(type) {
  const iconMap = {
    car: 'car',
