...
const getProfilePhotoPath = (userId) => `${FileSystem.documentDirectory}profile/${userId || 'guest'}/avatar.jpg`;
 ...
 */
export async function cacheProfilePhoto(remoteUri, userId) {
  try {
    if (!remoteUri || !userId) {
      return null;
    }
    ...
    if (existingCache) {
      ...
      if (fileInfo.exists) {
        ...
        console.log('[CACHE] Photo de profil chargée depuis le cache:', existingCache);
    ...
    const directory = `${FileSystem.documentDirectory}profile/${userId || 'guest'}`;
    ...
    try {
      ...
      console.log('[CACHE] Photo de profil téléchargée et mise en cache:', destination);
      return destination;
    } catch (downloadError) {
 ...
 */
export async function loadAndCacheProfilePhoto(remoteUri, userId) {
  try {
    if (!remoteUri || !userId) {
      return null;
    }
    ...
    if (cachedPhoto) {
      ...
      if (fileInfo.exists) {
        ...
        console.log('[CACHE] Photo de profil chargée depuis le cache:', cachedPhoto);
        return cachedPhoto;
      }
    }
 ...
 * @param {string} userId - ID de l'utilisateur
 */
export async function clearProfilePhotoCache(userId) {
  try {
    const cacheKey = `${PROFILE_PHOTO_CACHE_KEY}:${userId}`;
    await AsyncStorage.removeItem(cacheKey);
