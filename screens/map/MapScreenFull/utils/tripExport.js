...
import { stopBackgroundLocationUpdatesAsync } from '../../../../services/BackgroundLocationService';



export async function handleShareExport(params) {
  const {
    isTracking,
    trackingPoints,
    flattenSegments,
...
export async function saveTripWithStepsExport(params) {
  const {
    compressedPolyline,
    simplifiedPoints,
    trackingPoints,
  ...
  try {
    ...
    let elevationGain = 0;
    let elevationLoss = 0;
    let maxAltitude = altitudeData.length > 0 ? altitudeData[0] : null;
    let minAltitude = altitudeData.length > 0 ? altitudeData[0] : null;
...
export async function saveTripToStorageExport({ tripData, setSavedTrips }) {
  try {
...
export async function loadTripsFromStorageExport({ setSavedTrips }) {
  try {
...
export async function deleteTripFromStorageExport({ tripId, setSavedTrips }) {
  try {
    const trips = await AsyncStorage.getItem('savedTrips');
    if (trips) {
      const parsedTrips = JSON.parse(trips);
      const filteredTrips = parsedTrips.filter((trip) => trip.id !== tripId);
