...
const useTripMetrics = ({
  ...
  const [speedHistory, setSpeedHistoryState] = useState([]);
  ...
  const setAltitudeDataSafe = useCallback((updater) => {
    setAltitudeData((prev) => (typeof updater === 'function' ? updater(prev) : updater));
  }, []);

  const setDrivingScore = useCallback((score) => {
    setDrivingScoreState(score);
  }, []);

  const setSpeedHistory = useCallback((updater) => {
    setSpeedHistoryState((prev) => (typeof updater === 'function' ? updater(prev) : updater));
  }, []);

  const formatDisplaySpeed = useCallback((speed) => {
  ...
  const calculateDistanceBetweenPoints = useCallback((pointA, pointB) => {
  ...
  const formatTripTime = useCallback((seconds) => {
