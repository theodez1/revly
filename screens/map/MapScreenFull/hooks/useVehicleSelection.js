...
const resolveVehicleName = (vehicleId, vehicles) => {
...
const useVehicleSelection = (userId) => {
    ...
    async ({ keepSelection = false } = {}) => {
      ...
      try {
        ...
        const loadedVehicles = await Promise.all(
          (loadedVehiclesRaw || []).map(async (vehicle) => {
            if (vehicle.photoUri) {
              return vehicle;
            }
  ...
  const getVehicleName = useCallback(
    (vehicleId) => resolveVehicleName(vehicleId, vehicles),
    [vehicles],
  );
