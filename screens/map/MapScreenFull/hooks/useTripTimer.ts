...
const sanitizeTimerState = (rawState: any) => {
...
const computeElapsedMs = (state: any) => {
  if (!state) return 0;
...
const computeElapsedSeconds = (state: any) => {
...
const useTripTimer = () => {
  ...
  const persistState = useCallback((nextState: any) => {
    AsyncStorage.setItem(TIMER_STORAGE_KEY, JSON.stringify(nextState)).catch(() => {});
  }, []);

  const syncInterval = useCallback((nextState: any) => {
  ...
  const commitState = useCallback((updater: any) => {
    setState((prevState) => {
  ...
  return useMemo(() => ({
    ...
    status: state.status,
    seconds,
    // Cast pour éviter des problèmes de comparaison littérale avec le type inféré
    isRunning: (state.status as string) === 'running',
    isPaused: (state.status as string) === 'paused',
    start,
    pause,
    resume,
    stop,
