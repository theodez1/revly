...
export default function DiscoverGroupsScreen({ navigation }) {
  ...
  const resultsContentStyle = filteredGroups.length ? styles.resultsContainer : styles.emptyStateContainer;

  const getFilterLabel = useCallback(
    (filterId) => {
      const option = filterOptions.find((opt) => opt.id === filterId);
      const selectedId = selectedFilters[filterId];
      const choice = option?.choices?.find((c) => c.id === selectedId);
      return choice?.label || option?.choices?.[0]?.label || 'Tous';
    },
    [selectedFilters]
  );
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.filtersContainer}>
        <View style={styles.filterHeaderRow}>
          <Text style={styles.filtersTitle}>Filtres rapides</Text>
          <TouchableOpacity onPress={() => setShowFilters((prev) => !prev)} style={styles.filtersToggle}>
            <Text style={styles.filtersToggleText}>{showFilters ? 'Masquer' : 'Afficher'}</Text>
            <Ionicons
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterTabs}>
          {filterOptions.map((filter) => (
            <TouchableOpacity
              key={filter.id}
              style={[styles.filterSummaryChip, activeFilter === filter.id && styles.filterSummaryChipActive]}
              onPress={() => {
                setActiveFilter(filter.id);
                setShowFilters(true);
              }}
            >
              <Text style={[styles.filterSummaryLabel, activeFilter === filter.id && styles.filterSummaryLabelActive]}>
                {filter.label}: {getFilterLabel(filter.id)}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          {filteredGroups.length ? (
            filteredGroups.map((group) => (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() =>
                  navigation.navigate('GroupDetail' as any, {
                    group: {
                      id: group.id,
                      name: group.name,
              ...
              >
                ...
                <View style={styles.cardFooter}>
                  <TouchableOpacity
                    ...
                    onPress={(e) => {
                      e.stopPropagation();
                      const isPrivate = group.isPrivate || group.is_private;

                      if (group.requestStatus === 'pending') {
                        handleCancelRequest(group.id);
                      } else {
                        // Groupe public ou privé : appeler handleJoinGroup qui gère les deux cas
                        handleJoinGroup(group.id, isPrivate);
                      }
                    }}
                  >
                    <Ionicons
                    ...
                    >
                      {group.requestStatus === 'pending'
                        ? 'Annuler'
                        : (group.isPrivate || group.is_private ? 'Demander à rejoindre' : 'Rejoindre')}
                    </Text>
                  </TouchableOpacity>
                </View>
              </TouchableOpacity>
