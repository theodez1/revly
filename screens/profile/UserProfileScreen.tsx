...
// Durée courte pour les mini tuiles (ex: 1h05, 45m)
...
// Helper functions moved outside component to allow stable references
function calculateWeekActivity(rides: any[]) {
  const activity: Record<number, boolean> = { 0: false, 1: false, 2: false, 3: false, 4: false, 5: false, 6: false };
...
export default function UserProfileScreen({ navigation }) {
  ...
  const statsSummary = useMemo(() => {
    const totalDistance = Number(globalStats?.totalDistance || 0);
    const totalDuration = Number(globalStats?.totalDuration || 0);
    const totalRides = Number(globalStats?.totalRides || 0);
  ...
  const loadUserProfile = useCallback(async () => {
    console.log(`🚀 [UserProfile] START loading for userId: ${userId}`);

    if (!userId) {
      console.warn('⚠️ [UserProfile] No userId provided, aborting');
      setIsLoading(false);
      return;
    }

    setIsLoading(true);
    // Note: On ne reset pas profile à null pour éviter le flash blanc si on refresh
    setIsFollowing(null);

    // ÉTAPE 1: Identité (Bloquant)
    let userProfile = null;
    try {
      console.log('1️⃣ [UserProfile] Fetching Profile & Follow status...');
      const [profileResult, followingResult] = await Promise.all([
        userProfileService.getUserProfile(userId),
        currentUser?.id && !isOwnProfile
          ? userRelationshipsService.isFollowing(currentUser.id, userId)
          : Promise.resolve({ isFollowing: false })
      ]);

      if (profileResult.error) {
        console.error('❌ [UserProfile] Critical Error loading profile:', profileResult.error);
        setIsLoading(false);
        return; // Échec critique
      }

      console.log('✅ [UserProfile] Profile loaded:', profileResult.profile?.username);

      // Mise à jour immédiate
      userProfile = profileResult.profile;
      setProfile(userProfile);
      setIsFollowing(followingResult.isFollowing || false);

      // Laisser un tick pour le rendu
      await new Promise(r => setTimeout(r, 0));

    } catch (error) {
      console.error('❌ [UserProfile] CRASH Stage 1:', error);
      setIsLoading(false);
      return;
    }

    // Si on est là, l'utilisateur voit son profil. Le reste charge en arrière-plan (Waterfall).

    // ÉTAPE 2: Stats (Non-bloquant)
    try {
      console.log('2️⃣ [UserProfile] Fetching Stats...');
      const statsResult = await userProfileService.getUserStats(userId);
      console.log('✅ [UserProfile] Stats loaded');

      const userStats = (statsResult.stats || {}) as {
        totalRides?: number;
        totalDistance?: number;
      };
      setGlobalStats({
        totalRides: userStats.totalRides || 0,
        totalDistance: userStats.totalDistance || 0,
        totalDuration: 0,
        averageSpeed: 0,
      });
    } catch (e) {
      console.warn('⚠️ [UserProfile] Error loading stats:', e);
    }

    // ÉTAPE 3: Social (Non-bloquant)
    try {
      console.log('3️⃣ [UserProfile] Fetching Follow Stats...');
      const followStatsResult = await userRelationshipsService.getFollowStats(userId);
      console.log('✅ [UserProfile] Follow stats loaded');
      setFollowStats(followStatsResult.stats || { followersCount: 0, followingCount: 0 });
    } catch (e) {
      console.warn('⚠️ [UserProfile] Error loading follow stats:', e);
    }

    // ÉTAPE 4: Groupes (Non-bloquant)
    try {
      console.log('4️⃣ [UserProfile] Fetching Groups...');
      const groupsResult = await userProfileService.getUserGroups(userId);
      console.log(`✅ [UserProfile] Loaded ${groupsResult.groups?.length || 0} groups`);
      setUserGroups(groupsResult.groups || []);
    } catch (e) {
      console.warn('⚠️ [UserProfile] Error loading groups:', e);
    }

    // ÉTAPE 5: Trajets (Non-bloquant, le plus lourd)
    try {
      console.log('5️⃣ [UserProfile] Fetching Rides...');
      const ridesResult = await userProfileService.getUserRides(userId, 100);
      const userRidesData = ridesResult.rides || [];
      console.log(`✅ [UserProfile] Loaded ${userRidesData.length} rides`);
      ...
      // Traitement des données (Polyline decode etc)
      const formattedRides = userRidesData.map((ride) => {
        ...
        let photos = [];
        if (ride.photos && Array.isArray(ride.photos)) {
          photos = ride.photos.map(photo => typeof photo === 'string' ? { uri: photo } : photo);
        }

        return {
          id: ride.id,
          name: ride.name || 'Trajet',
          description: ride.description || '',
          startTime: ride.start_time || ride.startTime,
          endTime: ride.end_time || ride.endTime,
          duration: ride.duration || 0,
          distance: ride.distance || 0,
          maxSpeed: ride.max_speed || ride.maxSpeed || 0,
          averageSpeed: ride.average_speed || ride.averageSpeed || 0,
          routeCoordinates: routeCoordinates,
          polyline: ride.polyline || '',
          vehicle: ride.vehicle || 'car',
          startCity: ride.start_city || ride.startCity,
          endCity: ride.end_city || ride.endCity,
          cities: ride.cities || [],
          photos: photos,
          userId: ride.user_id || ride.userId || userId,
          userName: ride.userName || profileDisplayName,
          userAvatar: ride.userAvatar || userProfile.avatar_url || null,
        };
      });
      ...
      const sortedRides = formattedRides.sort((a, b) => {
        ...
        return getTimestamp(b.endTime || b.startTime) - getTimestamp(a.endTime || a.startTime);
      });

      setUserRides(sortedRides);

      if (sortedRides.length > 0) {
        setRecentDistances(computeLast7DaysDistances(sortedRides));
        setTrophies(calculateTrophies(sortedRides));
        setWeekStreak(calculateWeekStreak(sortedRides));
        setWeekActivity(calculateWeekActivity(sortedRides));
      }
    } catch (e) {
      console.warn('⚠️ [UserProfile] Error loading rides:', e);
    } finally {
      console.log('🏁 [UserProfile] Load sequence FINISHED');
      setIsLoading(false);
    }
  }, [userId, currentUser?.id, isOwnProfile]);

  // Handler pour toggle follow
  const handleToggleFollow = useCallback(async () => {
    if (!currentUser?.id || isOwnProfile || isTogglingFollow) return;

    setIsTogglingFollow(true);
    try {
      const { isFollowing: newFollowingState, error } = await userRelationshipsService.toggleFollow(
        currentUser.id,
        userId
      );
      ...
      const { stats: followStatsData } = await userRelationshipsService.getFollowStats(userId);
      setFollowStats(followStatsData);
    } catch (error) {
      console.error('Erreur inattendue toggle follow:', error);
    } finally {
      setIsTogglingFollow(false);
    }
  }, [currentUser?.id, userId, isOwnProfile, isTogglingFollow]);
  ...
  // Pas de chargement de véhicules pour les autres utilisateurs
  const loadVehicles = async () => {
    // Stub pour compatibilité avec la logique partagée, aucun chargement ici
    return [];
  };



  // Ne pas charger les véhicules pour un autre utilisateur

  // Pas de chargement de profileData depuis AsyncStorage pour les autres utilisateurs

  // Chargement initial - Supprimé car useFocusEffect gère déjà le chargement
  // useEffect(() => {
  //   loadUserProfile();
  // }, [loadUserProfile]);
  ...
  // Fonctions de gestion de véhicules désactivées pour les autres utilisateurs
  ...
  const handleSaveVehicle = async () => {
    ...
    try {
      ...
      if (editingVehicle) {
        ...
        // Plus besoin de photoUri côté client, photoUrl suffit pour l'UI
  ...
  // Fonction pickProfilePhoto désactivée pour les autres utilisateurs
    ...
    return (
      <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
        <View style={[styles.topHeader, { paddingTop: Math.max(insets.top, 8) }]}>
          <TouchableOpacity onPress={() => navigation?.goBack()} style={styles.headerChevron}>
    ...
    <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
      ...
      <View style={[styles.topHeader, { paddingTop: Math.max(insets.top, 8) }]}>
        ...
        <Text style={styles.headerUsername} numberOfLines={1}>
          {profile?.username || profile?.first_name || 'Utilisateur'}
        </Text>
        <TouchableOpacity style={styles.headerMore}>
          <Ionicons name="ellipsis-horizontal" size={24} color="#111827" />
        </TouchableOpacity>
      ...
      {/* Header fixe */}
      <View style={[styles.fixedHeader, { paddingTop: 8 }]}>
        ...
        <View style={styles.profileHeaderSection}>
          ...
          {currentUser?.id && userId && currentUser.id !== userId && isFollowing !== null && (
            <View style={styles.headerButtonsRight}>
              <TouchableOpacity
                style={[styles.followButtonHeader, isFollowing && styles.followButtonHeaderFollowing]}
              ...
              >
                {isTogglingFollow ? (
                  ...
                  <Text style={[styles.followButtonHeaderText, isFollowing && styles.followButtonHeaderTextFollowing]}>
      ...
      >
        ...
        {/* Page 2: Véhicules - Masqué pour les autres utilisateurs */}
        <ScrollView key="1" showsVerticalScrollIndicator={false} style={styles.contentScroll}>
          <View style={styles.emptyPostsContent}>
            <Text style={styles.emptyStateText}>Véhicules privés</Text>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // Nouveau layout: cartes de trajets en colonne
  feedCardsColumn: {
    paddingHorizontal: 16,
    paddingVertical: 4,
    gap: 12,
  ...
  // bottomSheetIndicator / sheetBackdrop / statsSection / statsHeaderRow déjà définis plus haut
  statsMainCardLarge: {
  ...
  followButtonHeaderTextFollowing: {
    color: '#111827',
  },
  groupsTabContent: {
    padding: 16,
  ...
  groupCardAvatarImage: {
    width: '100%',
    height: '100%',
  },
  groupCardAvatarPlaceholder: {
    width: '100%',
    height: '100%',
    justifyContent: 'center',
    alignItems: 'center',
  ...
  // postCardPlaceholder déjà défini plus haut pour les cartes de trajets
  postCardLikeBadge: {
    position: 'absolute',
    bottom: 4,
    right: 4,
  ...
  // START COPY OF NEW STATS STYLES
  // Performance Styles
  performanceRow: {
    flexDirection: 'row',
    gap: 12,
