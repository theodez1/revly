...
export default function UserProfileScreen({ navigation }) {
  ...
  const loadUserProfile = useCallback(async () => {
    ...
    setIsFollowing(null); // Reset état follow pour éviter flicker

    try {
      // ÉTAPE 1: Charger d'abord les données critiques (profil + état follow) pour affichage immédiat
      ...
      // Vérifier les erreurs du profil
      ...
      setIsLoading(false); // Les données critiques sont chargées, on peut afficher

      // ÉTAPE 2: Charger le reste en parallèle (stats, trajets, groupes, follow stats)
      const [
        statsResult,
        ridesResult,
        followStatsResult,
        ...
        userProfileService.getUserRides(userId, 100),
        userRelationshipsService.getFollowStats(userId),
        userProfileService.getUserGroups(userId)
      ]);
      ...
      const formattedRides = userRidesData.map((ride) => {
        ...
        // Convertir les photos si elles sont en format string
        let photos = [];
        if (ride.photos) {
          if (Array.isArray(ride.photos)) {
            photos = ride.photos.map(photo => {
      ...
      // Calculer les trophées, streaks, etc.
      const distances = computeLast7DaysDistances(sortedRides);
      setRecentDistances(distances);
      const calculatedTrophies = calculateTrophies(sortedRides);
      setTrophies(calculatedTrophies);
      const streak = calculateWeekStreak(sortedRides);
      setWeekStreak(streak);
      const weekDays = calculateWeekActivity(sortedRides);
      setWeekActivity(weekDays);
    } catch (error) {
      console.error('Erreur chargement profil utilisateur:', error);
      setIsLoading(false); // Arrêter le loading même en cas d'erreur
    }
  }, [userId, currentUser?.id, isOwnProfile, computeLast7DaysDistances, calculateTrophies, calculateWeekStreak, calculateWeekActivity]);
    ...
    <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
      ...
      <View style={styles.tabsRow}>
        ...
        <TouchableOpacity style={styles.tabItem} onPress={() => goToPage(2)}>
          <Ionicons
            name={currentPage === 2 ? 'people' : 'people-outline'}
            size={24}
      ...
      >
        ...
        <ScrollView key="2" showsVerticalScrollIndicator={false} style={styles.contentScroll}>
          <View style={styles.groupsTabContent}>
            {userGroups && userGroups.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      <View style={styles.groupCardAvatar}>
                        {group.avatar ? (
                          <Image source={{ uri: group.avatar }} style={styles.groupCardAvatarImage} />
                        ) : (
                          <View style={styles.groupCardAvatarPlaceholder}>
              ...
              <View style={styles.emptyGroupsContent}>
                ...
                <Text style={styles.emptyGroupsText}>Aucun groupe</Text>
                <Text style={styles.emptyGroupsSubtext}>
                  Cette personne n'est membre d'aucun groupe
                </Text>
              </View>
            )}
          </View>
