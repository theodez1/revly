 ...
 */
const TrackingLogsScreen = ({ navigation, route }) => {
    ...
    const loadSessions = async () => {
        try {
            setIsLoading(true);
            const loadedSessions = await trackingLogger.getSessions();
    ...
    const handleSelectSession = async (sessionId) => {
        try {
            ...
            const report = await trackingLogger.generateDiagnosticReport(sessionId);
    ...
    const handleExport = async (sessionId, format = 'txt') => {
        try {
            const filepath = await trackingLogger.exportSessionToFile(sessionId, format);
            Alert.alert(
                'Export réussi',
                `Logs exportés vers:\n${filepath}`,
                [
