...
const RideCarousel: React.FC<any> = React.memo(
  ({ ride, carouselIndex, onIndexChange, onCardPress }: any) => {
  ...
  const renderRideItem = ({ item: ride, index }) => {
    ...
    return (
      <View style={[styles.runCard, index > 0 && styles.runCardMargin]}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (ride.userId && ride.userId !== userId && ride.userId !== 'default') {
                (navigation as any).navigate('UserProfile', { userId: ride.userId });
              }
            }}
        ...
        {/* Actions (Like, Comment, Share) - Style Instagram */}
        <View style={styles.runInteractionContainer}>
          <View style={styles.runInteractionRow}>
            <View style={styles.runInteractionLeft}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={1}
              >
                <Ionicons name="chatbubble-outline" size={26} color="#0F172A" style={styles.iconThick} />
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.runInteractionItem}
                ...
                activeOpacity={1}
              >
                <Ionicons name="paper-plane-outline" size={26} color="#0F172A" style={styles.iconThick} />
              </TouchableOpacity>
            </View>
            <TouchableOpacity style={styles.runInteractionItem} activeOpacity={0.5}>
              <Ionicons name="bookmark-outline" size={26} color="#0F172A" style={styles.iconThick} />
            </TouchableOpacity>
          </View>

          {/* Affichage des likes en texte sous les icônes */}
          {ride.likesCount > 0 && (
            <Text style={styles.likesCountText}>
              {ride.likesCount === 1 ? '1 J\'aime' : `${ride.likesCount.toLocaleString('fr-FR')} J'aime`}
            </Text>
          )}

          {/* Affichage des commentaires si > 0 */}
          {ride.commentsCount > 0 && (
            <TouchableOpacity onPress={() => handleComment(ride)} activeOpacity={0.7}>
              <Text style={styles.viewCommentsText}>
                Voir les {ride.commentsCount} commentaire{ride.commentsCount > 1 ? 's' : ''}
              </Text>
            </TouchableOpacity>
          )}
        </View>
          ...
          <BottomSheetView style={styles.optionsSheetContent}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                if (selectedRide?.userId) {
                  (navigation as any).navigate('UserProfile', { userId: selectedRide.userId });
                }
              }}
            >
              <Ionicons name="person-circle-outline" size={20} color="#1F2937" />
              <Text style={styles.optionsSheetItemText}>Voir le profil</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                Alert.alert(
                  'Signalement envoyé',
                  'Nous avons bien reçu votre signalement. Notre équipe de modération va examiner ce contenu.',
                  [{ text: 'OK' }]
                );
              }}
            >
              <Ionicons name="flag-outline" size={20} color="#FF3B30" />
              <Text style={styles.optionsSheetItemTextDestructive}>Signaler</Text>
            </TouchableOpacity>
          </BottomSheetView>
        </BottomSheetModal>
