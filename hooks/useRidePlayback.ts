...
const getBearing = (p1: Coordinate, p2: Coordinate): number => {
    const φ1 = (p1.latitude * Math.PI) / 180;
    const φ2 = (p2.latitude * Math.PI) / 180;
    const Δλ = ((p2.longitude - p1.longitude) * Math.PI) / 180;
...
const interpolatePosition = (p1: Coordinate, p2: Coordinate, t: number): Coordinate => {
    return {
...
interface UseRidePlaybackResult {
    isPlaying: boolean;
    progress: number;
...
export const useRidePlayback = (routeCoordinates: Coordinate[]): UseRidePlaybackResult => {
