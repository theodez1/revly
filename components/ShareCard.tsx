...
interface ShareCardProps {
  ride: Partial<Ride> & { routeCoordinates?: Coordinate[] };
  variant?: string;
}

export default function ShareCard({ ride, variant = 'route-stats' }: ShareCardProps) {
  ...
  const buildRoutePath = (coords: Coordinate[], width: number, height: number, padding = 12) => {
  ...
  const showRoute = variant !== 'stats-only';
  const showStats = variant !== 'mini-route';
