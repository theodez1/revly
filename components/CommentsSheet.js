...
import { View, Text, StyleSheet, TouchableOpacity, Image, Keyboard, Platform, useWindowDimensions } from 'react-native';
 ...
 */
const CommentsSheet = React.forwardRef(({ type = 'ride', entityId, onCommentAdded }, ref) => {
    ...
    const handleSend = useCallback(async () => {
        ...
        try {
            ...
            const newComment = {
                ...
                user: {
                    name: result.comment.author,
                    avatar: result.comment.avatar,
                    id: result.comment.author_id
                },
    ...
    }, [commentText, entityId, currentUser, type, onCommentAdded]);
        ...
        return (
            <View style={styles.inputAvatarPlaceholder}>
                <Text style={styles.inputAvatarInitial}>
                    {(profile?.full_name || currentUser?.email || 'M').charAt(0).toUpperCase()}
                </Text>
            </View>
        );
    };
