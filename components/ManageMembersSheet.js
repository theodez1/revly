...
const ManageMembersSheet = ({ sheetRef, group, onMembersUpdated }) => {
  const { user } = useAuth();
  const [members, setMembers] = useState([]);
  const [invitations, setInvitations] = useState([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('members'); // 'members' ou 'invitations'
  ...
  const loadData = async () => {
    try {
      ...
      const isAdmin = group.createdBy === user?.id || 
                     membersResult.members?.some(m => m.id === user?.id && m.role === 'owner');
      ...
      if (isAdmin) {
        ...
        // Filtrer pour ce groupe uniquement
    ...
    Alert.alert(
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const result = await GroupsService.leaveGroup(group.id, memberId);
              if (result.error) {
                throw result.error;
              }
              await loadData();
  ...
  const isAdmin = isOwner || currentMember?.role === 'owner';
